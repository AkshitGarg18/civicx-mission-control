...
function NavList({
  ...
  return (
    <nav className="flex flex-col gap-1">
      <p className="mono-label px-3 pb-2 text-muted-foreground">INNOVATION COMMAND</p>
      {industryNav.map((item, i) => {
        ...
        return (
          ...
          >
            {isActive && (
              <motion.span
                layoutId="industry-nav-active"
                className="absolute inset-0 rounded-xl border border-warn/30 bg-warn/10"
            ...
            <Icon
              className={cn(
                ...
                isActive ? "text-warn" : "text-muted-foreground group-hover:text-warn",
              )}
            />
            <span className="relative flex-1 font-medium">{item.label}</span>
            {item.id === "opportunities" && signals > 0 && (
              <span className="relative rounded-md border border-warn/40 bg-warn/10 px-1.5 py-0.5 font-mono text-[9px] tracking-[0.12em] text-warn">
                {signals}
              </span>
            )}
          </motion.button>
  ...
  return (
    <div className="space-y-4">
      <div className="glass-soft rounded-xl px-3 py-3">
        <p className="mono-label text-muted-foreground">INNOVATION NETWORK</p>
...
/** Persistent industry console navigation with a mobile drawer. */
